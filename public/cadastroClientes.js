document.addEventListener("DOMContentLoaded", () => {
    ...
    const FIXED_EMAIL = "logistica@nexointernational.com.br"; // altere se quiser outro e-mail padrão
    ...
    // Por enquanto não vamos validar nada para não travar a abertura do modal
    function verificarCamposObrigatorios() {
        return true;
    }

    // Função para pegar a razão social com base no name do input
    function getRazaoSocial() {
        const razaoInput = document.querySelector('input[name="razao_social"]');
        if (!razaoInput) return "Cliente";
        return razaoInput.value.trim() || "Cliente";
    }

    // Função para gerar o PDF automaticamente
    async function gerarPDF() {
        // Vamos gerar o PDF apenas da folha (sheet), que é o formulário
        const content = document.querySelector('.sheet');
        if (!content) {
            alert('Não foi possível encontrar o conteúdo para gerar o PDF (.sheet).');
            return null;
        }

        const razaoSocial = getRazaoSocial();
        ...
        try {
            hideFeedback();
            buttonPdf.disabled = true; // evita clique duplo
    ...
    // Ao clicar no botão "ENVIAR E-MAIL COM ANEXOS"
    buttonPdf.addEventListener('click', async (event) => {
        ...
        // Gera o PDF e, se der certo, abre o modal de e-mail
        await gerarPDF();
        if (generatedPdfFile) {
            additionalFiles = [];
    ...
    sendEmailButton.addEventListener('click', async () => {
        ...
        try {
            ...
            // Razão social de novo (usando o name)
            const razaoSocial = getRazaoSocial();
